...
const SOURCE_PATTERNS: PatternRule[] = [
  ...
  },
  {
    ...
    description: "Trading or transfers may be paused by an admin — verify who controls pause.",
    severity: "medium",
    pattern: /\bwhenNotPaused\b|\bwhenPaused\b|\b_pause\b|\bunpause\b|Pausable/i,
  },
  {
    id: "proxy-admin",
    category: "proxy",
    title: "Proxy / upgrade admin",
    description: "Upgradeable pattern detected — implementation can change without notice.",
    severity: "high",
    pattern: /Upgradeable|UUPSUpgradeable|TransparentUpgradeableProxy|proxyAdmin|_authorizeUpgrade/i,
  },
  {
    id: "fee-on-transfer",
    category: "trading",
    title: "Fee-on-transfer pattern",
    description: "Balance adjustments on transfer — DEX integrations may behave unexpectedly.",
    severity: "medium",
    pattern: /feeOnTransfer|_transfer.*fee|takeFee|amount.*fee/i,
  },
  {
    id: "low-level-call",
    category: "code",
    title: "Low-level external calls",
    description: "Uses call/delegatecall to arbitrary targets — review targets and reentrancy.",
    severity: "medium",
    pattern: /\.call\{[^}]*\}\(|\.call\(|functionCall\(/i,
  },
  {
    id: "outdated-solidity",
    category: "code",
    title: "Outdated Solidity version",
    description: "Compiler below 0.8.x — no built-in overflow checks unless using SafeMath.",
    severity: "low",
    pattern: /pragma solidity \^?0\.[0-5]\./i,
  },
  {
    id: "external-call-transfer",
    category: "trading",
    title: "External call before state update",
    description: "Pattern may indicate reentrancy risk — verify checks-effects-interactions.",
    severity: "medium",
    pattern: /\.call\{value:|\.transfer\(|send\(/i,
  },
];
