    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-black via-dark-900 to-black py-16 px-4">
            <div className="max-w-4xl mx-auto">
                    ...
                    <form onSubmit={handleAnalyze} className="space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <button
                                    ...
                                    className={`flex items-center justify-center gap-2 p-4 rounded-lg border-2 transition-all ${platform === "github"
                                        ...
                                        : "border-border bg-dark-800/50 text-gray-400 hover:border-accent/50"
                                        }`}
                                >
                        ...
                        <div>
                            ...
                            <input
                                ...
                                required
                                className="w-full px-4 py-3 bg-dark-800/50 border border-border rounded-lg text-foreground placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent transition-all"
                            />
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                            <button
                                ...
                                className={`flex items-center gap-2 px-4 py-2 rounded-lg border transition-all ${useToken
                                    ? "border-accent bg-accent/10 text-accent"
                                    : "border-border bg-dark-800/50 text-gray-400"
                                    }`}
                            >
                        ...
                        <button
                            ...
                            className="w-full px-6 py-3 bg-gradient-to-r from-accent to-primary text-foreground font-medium rounded-lg hover:shadow-lg hover:shadow-accent/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
                            {loading ? (
                                <>
                ...
                {/* Results */}
                {result && (
                    <div className="mt-8 glass-card rounded-2xl p-8 shadow-2xl border border-border animate-fadeIn">
                        <div className="flex items-center gap-3 mb-6">
                            <CheckCircle2 className="w-6 h-6 text-green-400" />
                            <h2 className="text-2xl font-bold text-foreground">Analysis Queued Successfully</h2>
                        </div>

                        <div className="space-y-4 mb-6">
                            <div className="flex justify-between text-sm">
                                <span className="text-gray-400">Repository:</span>
                                <span className="text-foreground font-medium">{result.repository}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                                <span className="text-gray-400">Branch:</span>
                                <span className="text-foreground font-medium">{result.branch}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                                <span className="text-gray-400">Files Found:</span>
                                <span className="text-foreground font-medium">{result.totalFilesFound}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                                <span className="text-gray-400">Files Analyzed:</span>
                                <span className="text-foreground font-medium">{result.filesAnalyzed}</span>
                            </div>
                        </div>

                        {/* Analysis Jobs */}
                        <div>
                            <h3 className="text-lg font-semibold text-foreground mb-4">Analysis Jobs</h3>
                            <div className="space-y-2">
                                {result.jobs.map((job) => (
                                    <div
                                        key={job.jobId}
                                        className="flex items-center justify-between p-4 bg-dark-800/50 rounded-lg border border-border hover:border-accent/50 transition-all"
                                    >
                                        <div className="flex-1">
                                            <p className="text-sm font-medium text-foreground">{job.filePath}</p>
                                            <p className="text-xs text-gray-400 mt-1">Job ID: {job.jobId}</p>
                                        </div>
                                        <button
                                            onClick={() => viewAnalysis(job.jobId)}
                                            className="px-4 py-2 bg-accent/10 text-accent hover:bg-accent hover:text-foreground rounded-lg transition-all text-sm font-medium"
                                        >
                                            View Results
                                        </button>
                                    </div>
                        ...
                        <button
                            ...
                            className="mt-6 w-full px-6 py-3 bg-dark-800/50 text-foreground font-medium rounded-lg border border-border hover:border-accent transition-all"
                        >
